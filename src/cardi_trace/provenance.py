...
class ProvenanceGraph:
    ...
    def relation(self, relation: str, source: str, target: str, **attributes: Any) -> ProvRelation:
        ...
        return item
    def used(self, activity_id: str, entity_id: str, **attributes: Any) -> ProvRelation:
        return self.relation("used", activity_id, entity_id, **attributes)
    def generated(self, entity_id: str, activity_id: str, **attributes: Any) -> ProvRelation:
        return self.relation("wasGeneratedBy", entity_id, activity_id, **attributes)
    def derived(self, entity_id: str, source_id: str, **attributes: Any) -> ProvRelation:
        return self.relation("wasDerivedFrom", entity_id, source_id, **attributes)
    def associated(self, activity_id: str, agent_id: str, **attributes: Any) -> ProvRelation:
        return self.relation("wasAssociatedWith", activity_id, agent_id, **attributes)
    def to_dict(self) -> dict[str, Any]:
        return {
            "prefix": {"prov": "http://www.w3.org/ns/prov#", "ct": "https://virelion.org/cardi-trace#"},
            "entity": {k: {"id": v.id, **v.attributes} for k, v in sorted(self.entities.items())},
            "activity": {k: {"id": v.id, "startedAtTime": v.started_at, **({"endedAtTime": v.ended_at} if v.ended_at else {}), **v.attributes} for k, v in sorted(self.activities.items())},
            "agent": {k: {"id": v.id, **v.attributes} for k, v in sorted(self.agents.items())},
    ...
    def upstream(self, entity_id: str) -> set[str]:
        return self._walk(entity_id, forward=False)
    def downstream(self, entity_id: str) -> set[str]:
        return self._walk(entity_id, forward=True)
...
def register_source(graph: ProvenanceGraph, source: str, *, source_type: str = "dataset", version: str | None = None, uri: str | None = None, **metadata: Any) -> str:
    source_id = f"source:{sha256_payload({'source': source, 'version': version})[:24]}"
    graph.entity(source_id, source=source, source_type=source_type, version=version, uri=uri, **metadata); return source_id

def register_file_entity(graph: ProvenanceGraph, path: str | Path, *, role: str = "artifact", **metadata: Any) -> str:
    p = Path(path); digest = sha256_file(p); entity_id = f"artifact:sha256:{digest}"
    graph.entity(entity_id, kind=ArtifactKind.FILE, name=p.name, digest=digest, size_bytes=p.stat().st_size, uri=str(p.resolve()), role=role, **metadata); return entity_id
