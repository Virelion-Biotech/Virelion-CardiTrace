...
class ProvenanceGraph:
    ...
    def entity(self,entity_id:str,**attributes:Any)->ProvEntity: item=ProvEntity(entity_id,attributes); self.entities[entity_id]=item; return item
    def activity(self,activity_id:str,*,started_at:str|None=None,ended_at:str|None=None,**attributes:Any)->ProvActivity: item=ProvActivity(activity_id,started_at or _now(),ended_at,attributes); self.activities[activity_id]=item; return item
    def agent(self,agent_id:str,**attributes:Any)->ProvAgent: item=ProvAgent(agent_id,attributes); self.agents[agent_id]=item; return item
    def relation(self,relation:str,source:str,target:str,**attributes:Any)->ProvRelation: item=ProvRelation(relation,source,target,attributes); self.relations.append(item) if item not in self.relations else None; return item
    ...
    def to_dict(self): return {"prefix":{"prov":"http://www.w3.org/ns/prov#","ct":"https://virelion.org/cardi-trace#"},"entity":{k:{"id":v.id,**v.attributes} for k,v in sorted(self.entities.items())},"activity":{k:{"id":v.id,"startedAtTime":v.started_at,**({"endedAtTime":v.ended_at} if v.ended_at else {}),**v.attributes} for k,v in sorted(self.activities.items())},"agent":{k:{"id":v.id,**v.attributes} for k,v in sorted(self.agents.items())},"relations":[asdict(r) for r in self.relations]}
    ...
    def _neighbors(self,node_id:str,forward:bool):
        out=set()
        for r in self.relations:
            if r.relation=="used":
                edge=(r.source,r.target)
            elif r.relation=="wasGeneratedBy":
                edge=(r.target,r.source)
            elif r.relation=="wasDerivedFrom":
                edge=(r.target,r.source)
            elif r.relation=="wasInformedBy":
                edge=(r.target,r.source)
            else:
                continue
            a,b=edge if forward else (edge[1],edge[0])
            if a==node_id: out.add(b)
        return out
    def _walk(self,node_id:str,forward:bool):
        seen={node_id}; frontier=[node_id]
        while frontier:
            current=frontier.pop()
            for nxt in self._neighbors(current,forward):
                if nxt not in seen: seen.add(nxt); frontier.append(nxt)
        return seen
    def upstream(self,entity_id): return self._walk(entity_id,False)
    def downstream(self,entity_id): return self._walk(entity_id,True)
...
def dataset_manifest(source,artifacts,*,dataset_id=None,version=None,organism=None,modality=None,condition=None,metadata=None):
    items=sorted((a.to_dict() for a in artifacts),key=lambda x:x["artifact_id"]); manifest={"schema_version":"2.0","dataset_id":dataset_id or "","source":source,"version":version,"organism":organism,"modality":modality,"condition":condition,"artifacts":items,"metadata":metadata or {}}
...
def graph_from_recorder(recorder):
    ...
    for run in recorder.runs:
        aid=f"activity:{run.run_id}"; end=datetime.fromtimestamp(run.finished_at,timezone.utc).isoformat() if run.finished_at else None; graph.activity(aid,started_at=datetime.fromtimestamp(run.started_at,timezone.utc).isoformat(),ended_at=end,component=run.component,operation=run.operation,status=run.status,parameters=run.parameters,environment=run.environment,code_identity=run.code_identity,metrics=run.metrics,tags=run.tags,metadata=run.metadata); graph.associated(aid,agent_id)
    ...
    for edge in recorder.lineage: graph.derived(f"artifact:{edge.target_id}",f"artifact:{edge.source_id}",lineage_relation=edge.relation,run_id=edge.run_id,**edge.metadata)
    return graph
