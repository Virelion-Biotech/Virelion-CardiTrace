...
class TraceEnvelope:
    ...
    def to_dict(self, *, include_signature=True):
        ...
        if include_signature and self.signature: body["signature"]=self.signature
        return body
    @property
...
def create_envelope(recorder, *, source, target, trace_id=None, metadata=None, secret: bytes|None=None):
    ...
    algorithm="hmac-sha256" if secret else None
    if algorithm: body["signature_algorithm"]=algorithm
    signature=sign_payload(body,secret) if secret else None
    return TraceEnvelope(**body,signature=signature)

def verify_envelope(envelope: TraceEnvelope, recorder, *, secret: bytes|None=None, require_signature=False) -> tuple[str,...]:
    issues=[]
